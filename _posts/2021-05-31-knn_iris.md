...
<img src="/img/knn_iris_files/knn_points.png" width = "35%" />
...
$$d(x,y) = \text{archav}\bigg(\text{hav}(\Delta\phi) + \cos{(x_\phi)}\cos{(y_\phi)}\text{hav}(\Delta \lambda)\bigg) = 2~\text{arcsin}\sqrt{\text{sin}^2\Big(\frac{\Delta\phi}{2}\Big) + \cos{(x_\phi)}\cos{(y_\phi)}\text{sin}^2\Big(\frac{\Delta\lambda}{2}\Big)}$$
