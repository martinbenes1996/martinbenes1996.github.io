...
In kNN we work with minimal distance. Selection of how to compute distance (*distance metric*) is not so trivial, the most intuitive and the one we shall use is the "as bird flies", called *Euclidean distance metric*. Its mathematical equation is
